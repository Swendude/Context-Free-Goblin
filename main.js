...
var $author$project$Grammar$addRule = F3(
	function (gram, sym, rProduction) {
		...
		var rules = gram.a;
		if (newProduction.$ === 'Ok') {
			var prod = newProduction.a;
			return $elm$core$Result$Ok(
				$author$project$Grammar$Grammar(
					A3(
						$elm$core$Dict$update,
						sym,
						$author$project$Grammar$addProduction(prod),
						rules)));
		} else {
			var error = newProduction.a;
...
var $author$project$Grammar$NoStartRule = {$: 'NoStartRule'};
...
var $author$project$Grammar$EmptyProduction = {$: 'EmptyProduction'};
var $author$project$Grammar$SymbolMissing = function (a) {
	return {$: 'SymbolMissing', a: a};
};
...
var $author$project$Grammar$pickRule = F3(
	function (seed, _v0, symbol) {
		var rules = _v0.a;
		var _v1 = A2($elm$core$Dict$get, symbol, rules);
		if (_v1.$ === 'Just') {
			var productions = _v1.a;
			var seedAndChoice = A2(
				$elm$random$Random$step,
				$elm_community$random_extra$Random$List$shuffle(productions),
				seed);
			var newseed = seedAndChoice.b;
			var choice = $elm$core$List$head(seedAndChoice.a);
			if (choice.$ === 'Just') {
				var prod = choice.a;
				return _Utils_Tuple2(
					newseed,
					$elm$core$Result$Ok(prod));
			} else {
				return _Utils_Tuple2(
					newseed,
					$elm$core$Result$Err($author$project$Grammar$EmptyProduction));
			}
		} else {
			return _Utils_Tuple2(
				seed,
				$elm$core$Result$Err(
					$author$project$Grammar$SymbolMissing(symbol)));
		}
	});
var $author$project$Grammar$prodPartToString = function (pp) {
	if (pp.$ === 'Symbol') {
		var s = pp.a;
		return '{' + (s + '}');
	} else {
...
var $author$project$Grammar$prodToString = function (prod) {
	return A2(
		$elm$core$String$join,
		'',
		A2($elm$core$List$map, $author$project$Grammar$prodPartToString, prod));
};
var $author$project$Grammar$RecursionError = {$: 'RecursionError'};
...
var $author$project$Grammar$isSymbol = function (prodPart) {
	if (prodPart.$ === 'Symbol') {
		return true;
	} else {
		return false;
...
var $author$project$Grammar$resolveProdPart = F3(
	function (gram, prodPart, _v0) {
		var seed = _v0.a;
		var curProd = _v0.b;
		if (prodPart.$ === 'Token') {
			var str = prodPart.a;
				...
				A2(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$author$project$Grammar$Token(str)
								]))
						]),
					curProd));
		} else {
			var sym = prodPart.a;
			var _v2 = A3($author$project$Grammar$pickRule, seed, gram, sym);
			var new_seed = _v2.a;
			var choice = _v2.b;
			return _Utils_Tuple2(
				new_seed,
...
var $author$project$Grammar$replaceSymbols = F4(
	function (seed, recursionCounter, gram, cur) {
		replaceSymbols:
		while (true) {
			var replaced = A2(
				$elm$core$Result$map,
				...
				$elm_community$result_extra$Result$Extra$combine(
					A3(
						$elm$core$List$foldr,
						$author$project$Grammar$resolveProdPart(gram),
						_Utils_Tuple2(seed, _List_Nil),
						cur).b));
			if (replaced.$ === 'Ok') {
				var newprod = replaced.a;
				if (recursionCounter > 100) {
					return $elm$core$Result$Err($author$project$Grammar$RecursionError);
				} else {
					if (A2($elm$core$List$any, $author$project$Grammar$isSymbol, newprod)) {
						var $temp$seed = seed,
							$temp$recursionCounter = recursionCounter + 1,
							$temp$gram = gram,
							$temp$cur = newprod;
						seed = $temp$seed;
						recursionCounter = $temp$recursionCounter;
						gram = $temp$gram;
						cur = $temp$cur;
						continue replaceSymbols;
					} else {
						return $elm$core$Result$Ok(newprod);
					}
				}
			} else {
				var err = replaced.a;
...
var $author$project$Grammar$generateSentence = F2(
	function (seed, _v0) {
		var rules = _v0.a;
		if (A2($elm$core$Dict$member, 'START', rules)) {
			var choice = A3(
				$author$project$Grammar$pickRule,
				seed,
				$author$project$Grammar$Grammar(rules),
				'START').b;
			if (choice.$ === 'Ok') {
				var prod = choice.a;
				return A2(
					$elm$core$Result$map,
					$author$project$Grammar$prodToString,
					A4(
						$author$project$Grammar$replaceSymbols,
						seed,
						0,
						$author$project$Grammar$Grammar(rules),
						prod));
...
var $author$project$Grammar$getRule = F3(
	function (_v0, sym, ix) {
		var rules = _v0.a;
		return A2(
...
var $author$project$Codec$tokenDecoder = A2(
	$elm$json$Json$Decode$map,
	$author$project$Grammar$Token,
	A2($elm$json$Json$Decode$field, 'str', $elm$json$Json$Decode$string));
var $author$project$Codec$prodPartFromType = function (str) {
	switch (str) {
		case 'symbol':
			return $author$project$Codec$symbolDecoder;
		case 'token':
			return $author$project$Codec$tokenDecoder;
		default:
			return $elm$json$Json$Decode$fail('Invalid type: ' + str);
	}
};
var $author$project$Codec$prodDecoder = $elm$json$Json$Decode$list(
	A2(
		$elm$json$Json$Decode$andThen,
		$author$project$Codec$prodPartFromType,
		A2($elm$json$Json$Decode$field, 'type', $elm$json$Json$Decode$string)));
var $author$project$Codec$prodsDecoder = $elm$json$Json$Decode$list($author$project$Codec$prodDecoder);
var $author$project$Codec$ruleDecoder = $elm$json$Json$Decode$list(
	A3(
		$elm$json$Json$Decode$map2,
		$elm$core$Tuple$pair,
		A2($elm$json$Json$Decode$field, 'symbol', $elm$json$Json$Decode$string),
		A2($elm$json$Json$Decode$field, 'productions', $author$project$Codec$prodsDecoder)));
var $author$project$Codec$rulesDecoder = A2($elm$json$Json$Decode$map, $elm$core$Dict$fromList, $author$project$Codec$ruleDecoder);
var $author$project$Codec$grammarDecoder = A2(
	$elm$json$Json$Decode$map,
	$author$project$Grammar$Grammar,
...
var $author$project$Grammar$cleanGrammar = function (_v0) {
	var rules = _v0.a;
	return $author$project$Grammar$Grammar(
		A2(
			$elm_community$dict_extra$Dict$Extra$removeWhen,
			F2(
				function (_v1, prods) {
					return $elm$core$List$isEmpty(prods);
				}),
			rules));
};
...
var $author$project$Grammar$removeRule = F3(
	function (_v0, sym, ix) {
		var rules = _v0.a;
		return $author$project$Grammar$cleanGrammar(
			$author$project$Grammar$Grammar(
				A3(
					$elm$core$Dict$update,
					sym,
		...
		switch (msg.$) {
			...
			case 'ProdChange':
				...
				return _Utils_Tuple2(
					...
					$elm$core$Platform$Cmd$none);
			case 'Save':
				if ((model.ntValue !== '') && (model.prodValue !== '')) {
					var _v1 = A3($author$project$Grammar$addRule, model.grammar, model.ntValue, model.prodValue);
					if (_v1.$ === 'Ok') {
						var gram = _v1.a;
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{grammar: gram, prodValue: ''}),
							$elm$core$Platform$Cmd$none);
					} else {
						var error = _v1.a;
						return _Utils_Tuple2(
					...
					_Utils_update(
						model,
						{
							output: A2(
								...
								'ERROR',
								A2(
									$author$project$Grammar$generateSentence,
									$elm$random$Random$initialSeed(s),
									model.grammar)),
							seed: s
						}),
					$elm$core$Platform$Cmd$none);
			...
			case 'ClearGrammar':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{grammar: $author$project$Grammar$empty, ntValue: 'START'}),
					$elm$core$Platform$Cmd$none);
			case 'CopyProduction':
				var _v2 = msg.a;
				var sym = _v2.a;
						...
						model,
						{
							prodValue: A2(
								...
								A2(
									$elm$core$Maybe$map,
									$author$project$Grammar$prodToString,
									A3($author$project$Grammar$getRule, model.grammar, sym, ix)))
						}),
					$elm$core$Platform$Cmd$none);
			case 'DeleteProduction':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							grammar: A3($author$project$Grammar$removeRule, model.grammar, sym, ix)
						}),
					$elm$core$Platform$Cmd$none);
			case 'GotDefaultGrammar':
				var res = msg.a;
				if (res.$ === 'Ok') {
					var grammarString = res.a;
					if (grammarString.$ === 'Just') {
						var gramString = grammarString.a;
						var _v6 = A2($elm$json$Json$Decode$decodeString, $author$project$Codec$grammarDecoder, gramString.grammar);
						if (_v6.$ === 'Ok') {
							var gram = _v6.a;
							return _Utils_Tuple2(
								_Utils_update(
							...
							return A2(
								$elm$core$Debug$log,
								$elm$core$Debug$toString('ENCODE ERROR'),
								_Utils_Tuple2(model, $elm$core$Platform$Cmd$none));
						}
					} else {
						return A2(
							$elm$core$Debug$log,
							$elm$core$Debug$toString('NO ROWS ERROR'),
							_Utils_Tuple2(model, $elm$core$Platform$Cmd$none));
					}
				} else {
					var error = res.a;
			...
			case 'SymbolHover':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							symbolHovered: $elm$core$Maybe$Just(sym)
						}),
					$elm$core$Platform$Cmd$none);
			case 'ExitSymbolHover':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{symbolHovered: $elm$core$Maybe$Nothing}),
			...
			case 'ScreenResize':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							screen: $mdgriffith$elm_ui$Element$classifyDevice(
								{height: h, width: w})
						}),
					$elm$core$Platform$Cmd$none);
			case 'SymbolFocus':
...
var $author$project$Main$GrammarRecord = F2(
	function (symbol, productions) {
		return {productions: productions, symbol: symbol};
	});
var $author$project$Main$grammarRecords = function (_v0) {
	var rules = _v0.a;
	return A2(
		$elm$core$List$map,
		function (_v1) {
			var sym = _v1.a;
			var prods = _v1.b;
			return A2($author$project$Main$GrammarRecord, sym, prods);
		},
		$elm$core$Dict$toList(rules));
};
var $author$project$Main$ExitSymbolHover = {$: 'ExitSymbolHover'};
var $author$project$Main$SymbolHover = function (a) {
	return {$: 'SymbolHover', a: a};
};
...
var $author$project$Main$ExitProductionHover = {$: 'ExitProductionHover'};
var $author$project$Main$ProductionHover = function (a) {
	return {$: 'ProductionHover', a: a};
};
...
var $author$project$Main$CopyProduction = function (a) {
	return {$: 'CopyProduction', a: a};
};
var $author$project$Main$DeleteProduction = function (a) {
	return {$: 'DeleteProduction', a: a};
};
			...
			[
				...
				_List_fromArray(
					[
						...
						$author$project$Main$DeleteProduction(
							_Utils_Tuple2(sym, ix)))
					]),
				...
				_List_fromArray(
					[
						...
						$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$fill),
						$mdgriffith$elm_ui$Element$Events$onMouseDown(
						$author$project$Main$CopyProduction(
							_Utils_Tuple2(sym, ix)))
					]),
	...
	function (sym, ix, hoveredProduction) {
		var hovered = _List_fromArray(
			[
				...
				$author$project$Main$ProductionHover(
					_Utils_Tuple2(sym, ix))),
				$mdgriffith$elm_ui$Element$Events$onMouseLeave($author$project$Main$ExitProductionHover),
				$mdgriffith$elm_ui$Element$below(
				$author$project$Main$productionDropDown(
					_Utils_Tuple2(sym, ix)))
			]);
		...
		if (hoveredProduction.$ === 'Nothing') {
			return _default;
		} else {
			var _v1 = hoveredProduction.a;
			var sym_ = _v1.a;
			var ix_ = _v1.b;
			return (_Utils_eq(sym, sym_) && _Utils_eq(ix, ix_)) ? hovered : _default;
		}
	});
var $mdgriffith$elm_ui$Internal$Flag$fontWeight = $mdgriffith$elm_ui$Internal$Flag$flag(13);
var $mdgriffith$elm_ui$Element$Font$bold = A2($mdgriffith$elm_ui$Internal$Model$Class, $mdgriffith$elm_ui$Internal$Flag$fontWeight, $mdgriffith$elm_ui$Internal$Style$classes.bold);
var $mdgriffith$elm_ui$Element$Font$underline = $mdgriffith$elm_ui$Internal$Model$htmlClass($mdgriffith$elm_ui$Internal$Style$classes.underline);
var $author$project$Main$hoveredSymbolStyle = _List_fromArray(
	[
...
var $author$project$Main$renderProdPart = F2(
	function (hovered, part) {
		var partPadding = $mdgriffith$elm_ui$Element$paddingEach(
			{bottom: 10, left: 0, right: 0, top: 10});
		if (part.$ === 'Symbol') {
			var production = part.a;
			var defaultSymbolStyle = _List_fromArray(
				[
					partPadding,
					$mdgriffith$elm_ui$Element$Font$bold,
					$mdgriffith$elm_ui$Element$Events$onMouseEnter(
					$author$project$Main$SymbolHover(production)),
					$mdgriffith$elm_ui$Element$Events$onMouseLeave($author$project$Main$ExitSymbolHover),
			...
			if (hovered.$ === 'Just') {
				var sym = hovered.a;
				return _Utils_eq(sym, production) ? A2(
					$mdgriffith$elm_ui$Element$el,
					_Utils_ap(defaultSymbolStyle, $author$project$Main$hoveredSymbolStyle),
					$mdgriffith$elm_ui$Element$text(production)) : A2(
					$mdgriffith$elm_ui$Element$el,
					defaultSymbolStyle,
					$mdgriffith$elm_ui$Element$text(production));
			} else {
				return A2(
					$mdgriffith$elm_ui$Element$el,
					defaultSymbolStyle,
			...
			return A2(
				$mdgriffith$elm_ui$Element$el,
				_List_fromArray(
					[partPadding, $mdgriffith$elm_ui$Element$Font$light]),
				$mdgriffith$elm_ui$Element$text(production));
		}
	});
...
var $author$project$Main$renderProduction = F5(
	function (symbol, symbolHovered, productionHovered, ix, prod) {
		return A2(
			$mdgriffith$elm_ui$Element$row,
			A3($author$project$Main$productionStyle, symbol, ix, productionHovered),
			A2(
				$elm$core$List$map,
				$author$project$Main$renderProdPart(symbolHovered),
				prod));
	});
var $author$project$Main$lgrey = A4($mdgriffith$elm_ui$Element$rgba255, 238, 238, 238, 1);
var $author$project$Main$mgrey = A4($mdgriffith$elm_ui$Element$rgba255, 212, 212, 212, 1);
var $author$project$Main$rowColor = function (ix) {
	return (!A2($elm$core$Basics$modBy, 2, ix)) ? $author$project$Main$lgrey : $author$project$Main$mgrey;
};
...
var $author$project$Main$grammarRow = F4(
	function (i, hoveredSymbol, hoveredProduction, production) {
		var hoveredRowStyle = function () {
			if (hoveredSymbol.$ === 'Nothing') {
				return _List_fromArray(
					[
						$mdgriffith$elm_ui$Element$Background$color(
						$author$project$Main$rowColor(i))
					]);
			} else {
				var hoveredSym = hoveredSymbol.a;
				return _Utils_eq(hoveredSym, production.symbol) ? _List_fromArray(
					[
						$mdgriffith$elm_ui$Element$Background$color($author$project$Main$goblinGreen)
					]) : _List_fromArray(
					[
				...
				[
					...
					A2(
						...
						_List_fromArray(
							[
								...
								$mdgriffith$elm_ui$Element$height($mdgriffith$elm_ui$Element$fill),
								$mdgriffith$elm_ui$Element$Events$onMouseEnter(
								$author$project$Main$SymbolHover(production.symbol)),
								$mdgriffith$elm_ui$Element$Events$onMouseLeave($author$project$Main$ExitSymbolHover)
							]),
						A2(
							...
							_List_fromArray(
								[
									$mdgriffith$elm_ui$Element$text(production.symbol)
								])))),
					A2(
					$mdgriffith$elm_ui$Element$el,
					_List_fromArray(
					...
					A2(
						...
						A2(
							...
							A3($author$project$Main$renderProduction, production.symbol, hoveredSymbol, hoveredProduction),
							production.productions)))
				]));
	});
var $author$project$Main$ClearGrammar = {$: 'ClearGrammar'};
...
var $author$project$Main$grammarView = F3(
	function (gram, symbolHovered, productionHovered) {
		var ruleRecords = $author$project$Main$grammarRecords(gram);
		return A2(
			$mdgriffith$elm_ui$Element$column,
			_List_fromArray(
				[
			...
			A2(
				...
				A2(
					...
					F2(
						function (i, gr) {
							return A4($author$project$Main$grammarRow, i, symbolHovered, productionHovered, gr);
						}),
					ruleRecords)));
	});
var $author$project$Main$NTermChange = function (a) {
	return {$: 'NTermChange', a: a};
};
var $author$project$Main$ProdChange = function (a) {
	return {$: 'ProdChange', a: a};
};
...
var $author$project$Grammar$getSymbols = function (_v0) {
	var rules = _v0.a;
	return $elm$core$Dict$keys(rules);
};
...
var $author$project$Main$symbolInputDropdown = function (model) {
	var row = function (sym) {
		return A2(
			$mdgriffith$elm_ui$Element$el,
			_List_fromArray(
				[
					...
					$mdgriffith$elm_ui$Element$Events$onMouseDown(
					$author$project$Main$SymbolSelected(sym)),
					$mdgriffith$elm_ui$Element$Events$onMouseEnter(
					$author$project$Main$SymbolHover(sym)),
					$mdgriffith$elm_ui$Element$Events$onMouseLeave($author$project$Main$ExitSymbolHover)
				]),
			A2(
				$mdgriffith$elm_ui$Element$paragraph,
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text(sym)
					])));
	};
	return A2(
		$mdgriffith$elm_ui$Element$column,
		...
		A2(
			...
			A2(
				$elm$core$List$filter,
				function (sym) {
					return A2($elm$core$String$startsWith, model.ntValue, sym);
				},
				$author$project$Grammar$getSymbols(model.grammar))));
};
...
var $author$project$Main$textInputStyle = _List_fromArray(
	[
		$mdgriffith$elm_ui$Element$Font$size(12),
		$mdgriffith$elm_ui$Element$Border$rounded(0)
	]);
var $author$project$Main$inputView = function (model) {
	var belowSymbolInput = model.symbolFocused ? $mdgriffith$elm_ui$Element$below(
		$author$project$Main$symbolInputDropdown(model)) : $mdgriffith$elm_ui$Element$below($mdgriffith$elm_ui$Element$none);
	return A2(
		$mdgriffith$elm_ui$Element$row,
		_List_fromArray(
			[
			...
			[
				...
				A2(
					...
					_Utils_ap(
						...
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$Events$onFocus($author$project$Main$SymbolFocus),
								$mdgriffith$elm_ui$Element$Events$onLoseFocus($author$project$Main$SymbolLoseFocus),
								belowSymbolInput,
								$mdgriffith$elm_ui$Element$Font$family(
								_List_fromArray(
									[
							...
							])),
					{
						label: model.showHelp ? A2(
							...
							A2(
								...
								_List_fromArray(
									[
										$mdgriffith$elm_ui$Element$text('Put a symbol here, there should always be a START symbol')
									]))) : $mdgriffith$elm_ui$Element$Input$labelHidden('Put a symbol here, there should always be a START symbol'),
						onChange: $author$project$Main$NTermChange,
					...
					$author$project$Main$textInputStyle,
					{
						label: model.showHelp ? A2(
							...
							A2(
								...
								_List_fromArray(
									[
										$mdgriffith$elm_ui$Element$text('Put some text here, use {} to mark symbols.')
									]))) : $mdgriffith$elm_ui$Element$Input$labelHidden('Put some text here, use {} to delimit symbols'),
						onChange: $author$project$Main$ProdChange,
						placeholder: $elm$core$Maybe$Just(
							A2(
								$mdgriffith$elm_ui$Element$Input$placeholder,
								_List_Nil,
								$mdgriffith$elm_ui$Element$text('You see a {monster}'))),
	...
	_List_fromArray(
		[
			...
			A2(
				...
				_List_fromArray(
					[
						$mdgriffith$elm_ui$Element$text('A tool for building and sharing random text generators.')
					])))
		]));
var $author$project$Main$view = function (model) {
	...
	return A2(
		...
		A2(
			...
			_List_fromArray(
				[
					$author$project$Main$titleView,
					A3($author$project$Main$grammarView, model.grammar, model.symbolHovered, model.productionHovered),
					$author$project$Main$inputView(model),
					$author$project$Main$outputView(model),
					A2(
					$mdgriffith$elm_ui$Element$el,
					_List_fromArray(
						[
					...
					A2(
						...
						_List_fromArray(
							[
								$mdgriffith$elm_ui$Element$text('Context Free Goblin is free open-source software. Made in Elm by @Swendude with <3')
							])))
				])));
};
